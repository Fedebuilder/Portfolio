  ...
  return (
    <section style={{ paddingBottom: '40px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px' }}>
        ...
        <span style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.14em', color: '#3b82c4', whiteSpace: 'nowrap' as const }}>{t('about.title')}</span>
        <div style={{ flex: 1, height: '1.5px', background: '#c8d3e0' }} />
      </div>
      ...
      <div className="card-pad" style={{ background: '#fff', border: '1px solid #c8d3e0', borderRadius: '20px', padding: '28px 32px', boxShadow: '0 2px 16px rgba(26,26,46,0.07)' }}>
        ...
        <div style={{ background: '#eef1f5', borderRadius: '14px', padding: '16px 20px', marginBottom: '24px' }}>
          <p style={{ fontSize: '12px', fontWeight: 700, color: '#2563ab', marginBottom: '8px', textTransform: 'uppercase' as const, letterSpacing: '0.06em' }}>{t('about.education_title')}</p>
        ...
        <div>
          <p style={{ fontSize: '11px', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.08em', color: '#9ca3af', marginBottom: '12px' }}>{t('about.fun_title')}</p>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {funItems.map((item, i) => (
              <p key={i} style={{ fontSize: '13px', color: '#5a6475', lineHeight: 1.5 }}>{item}</p>
            ))}
          </div>
        </div>
      </div>
