  ...
  return (
    <section style={{ paddingBottom: '48px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', marginBottom: '28px' }}>
        ...
        <span style={{ fontSize: '12px', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.14em', color: '#3b82c4', whiteSpace: 'nowrap' as const }}>{t('projects.title')}</span>
        <div style={{ flex: 1, height: '1.5px', background: '#c8d3e0' }} />
      </div>
          ...
          <a key={p.key} href={p.link} target="_blank" rel="noreferrer" style={{ textDecoration: 'none', display: 'block' }}>
            ...
            >
              ...
              <div style={{ padding: '20px 24px' }}>
                <p style={{ fontSize: '10px', fontWeight: 700, textTransform: 'uppercase' as const, letterSpacing: '0.1em', color: '#3b82c4', marginBottom: '5px' }}>{t(p.type_key)}</p>
                <h2 style={{ fontFamily: "'Fraunces', serif", fontSize: '18px', fontWeight: 600, color: '#111827', marginBottom: '8px', letterSpacing: '-0.01em' }}>{t(p.title_key)}</h2>
